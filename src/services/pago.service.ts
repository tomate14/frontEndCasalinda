...
export class PagosService {
  ...
  public getPagoByIdPedido(idPedido:string): Observable<PagosPorPedido> {
      return this.httpClient.get<PagosPorPedido>(`http://127.0.0.1:8080/pago/${idPedido}`);
  }

  public postPago(pago:Pago): Observable<Pago> {
    return this.httpClient.post<Pago>("http://127.0.0.1:8080/pago",pago);    
  }
  public putPago(idPago:string, pago:Pago): Observable<Pago> {
    return this.httpClient.put<Pago>(`http://127.0.0.1:8080/pago/${idPago}`,pago);    
  }

  public deletePagoByIdPago(idPago:string): Observable<Pago> {
    return this.httpClient.delete<Pago>(`http://127.0.0.1:8080/pago/${idPago}`);
  }
  
  public getCajaByDate(fechaInicio:string, fechaFin:string): Observable<Pago[]> {
    return this.httpClient.get<Pago[]>(`http://127.0.0.1:8080/pago/caja/${fechaInicio}/${fechaFin}`);
  }
}